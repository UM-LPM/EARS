...
public class RatingEnsemble extends MORatingBenchmark<Double, DoubleMOTask, DoubleMOProblem>{
    ...
    public RatingEnsemble(List<IndicatorName> indicators, double drawLimit) {
        super(indicators);
    ...
    public RatingEnsemble(ArrayList<IndicatorName> indicators, double[] weights, double drawLimit) {
...
//    	problems.add(new WFG9(2));
    	...
    	for (DoubleMOProblem moProblem : problems) {
    		registerTask(stopCriterion, maxEvaluations, timeLimit, maxIterations, 0.0001, moProblem);
		}
    }
}
