...
public class EnvironmentalSelection<Type> {
	...
	private List<MOSolutionBase<Type>> findExtremePoints(List<MOSolutionBase<Type>> population) {
		...
		MOSolutionBase<Type> min_indv = null;
		for (int f=0; f < numberOfObjectives; f+=1)
		{
	...
	public List<Double> constructHyperplane(List<MOSolutionBase<Type>> population, List<MOSolutionBase<Type>> extreme_points) {
		...
		else
		{
			...
			for (MOSolutionBase<Type> s : extreme_points)
			{
	...
	public void normalizeObjectives(List<MOSolutionBase<Type>> population, List<Double> intercepts, List<Double> ideal_point) {
	...
	int FindNicheReferencePoint()
	{
		...
		return min_rps.get(min_rps.size() > 1 ? Util.rnd.nextInt(min_rps.size()):0);
	}
	...
	public List<MOSolutionBase<Type>> execute(List<MOSolutionBase<Type>> source) {
		...
		List<MOSolutionBase<Type>> extreme_points = 		findExtremePoints(source);
		...
		while (source.size() < this.solutionsToSelect)
		{
			...
			MOSolutionBase<Type> chosen = SelectClusterMember(this.referencePoints.get(min_rp));
