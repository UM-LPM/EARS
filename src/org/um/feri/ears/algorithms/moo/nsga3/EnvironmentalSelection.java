...
public class EnvironmentalSelection<Type extends Number> {
    ...
    private List<List<NumberSolution<Type>>> fronts;
    private int solutionsToSelect;
    private List<ReferencePoint<Type>> referencePoints;
    private int numberOfObjectives;


    public EnvironmentalSelection(List<List<NumberSolution<Type>>> fronts, int solutionsToSelect, List<ReferencePoint<Type>> referencePoints, int numberOfObjectives) {
        this.fronts = fronts;
    ...
    public List<Double> translateObjectives(List<NumberSolution<Type>> population) {
        ...
        for (int f = 0; f < numberOfObjectives; f += 1) {
            ...
            for (List<NumberSolution<Type>> list : fronts) {
                for (NumberSolution<Type> s : list) {
    ...
    // ----------------------------------------------------------------------
    private double ASF(NumberSolution<Type> s, int index) {
    ...
    // ----------------------------------------------------------------------
    private List<NumberSolution<Type>> findExtremePoints(List<NumberSolution<Type>> population) {
        List<NumberSolution<Type>> extremePoints = new ArrayList<>();
        NumberSolution<Type> minIndv = null;
        for (int f = 0; f < numberOfObjectives; f += 1) {
            double minASF = Double.MAX_VALUE;
            for (NumberSolution<Type> s : fronts.get(0)) { // only consider the individuals in the first front
                double asf = ASF(s, f);
                if (asf < minASF) {
                    minASF = asf;
                    minIndv = s;
    ...
    public List<Double> constructHyperplane(List<NumberSolution<Type>> population, List<NumberSolution<Type>> extremePoints) {
        ...
        if (duplicate) // cannot construct the unique hyperplane (this is a casual method to deal with the condition)
        {
            ...
            for (NumberSolution<Type> s : extremePoints) {
    ...
    public void normalizeObjectives(List<NumberSolution<Type>> population, List<Double> intercepts, List<Double> idealPoint) {
        for (int t = 0; t < fronts.size(); t += 1) {
            for (NumberSolution<Type> s : fronts.get(t)) {
    ...
    public void associate(List<NumberSolution<Type>> population) {
    ...
    // ----------------------------------------------------------------------
    NumberSolution<Type> SelectClusterMember(ReferencePoint rp) {
        NumberSolution<Type> chosen = null;
    ...
    public List<NumberSolution<Type>> execute(List<NumberSolution<Type>> source) {
        ...
        List<NumberSolution<Type>> extremePoints = findExtremePoints(source);
        ...
        while (source.size() < this.solutionsToSelect) {
            ...
            NumberSolution<Type> chosen = SelectClusterMember(this.referencePoints.get(minRp));
    ...
    public void setAttribute(NumberSolution<Type> solution, List<Double> value) {
        solution.setAttribute(getAttributeID(), value);
    }


    public List<Double> getAttribute(NumberSolution<Type> solution) {
