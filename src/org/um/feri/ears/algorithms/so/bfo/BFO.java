...
public class BFO extends Algorithm {
	int p, S, Nc, Ns, Nre, Ned, Sr;
	double Ped, Ci;
	ArrayList<Double> C;
	ArrayList<Bacteria> swarm;
	Bacteria best, tmp;
	Random rnd;
...
//		this(60, 10, 10, 5, 2, 0.01);
	...
	public void initSwarm(Task task) throws StopCriteriaException {
		swarm.clear();
		best = null;
		for (int i = 0; i < S; i++) {
	...
	public void reproduction() throws StopCriteriaException {
	...
	public Bacteria eleminationAndDispersal(Bacteria b, Task task, int i) throws StopCriteriaException {
	...
	public DoubleSolution execute(Task task) throws StopCriteriaException {
		...
		while (!task.isStopCriteria()) {    
			for (int l = 0; l < Ned; l++) {
				...
				for (int i = 0; i < S; i++) {
					tmp = eleminationAndDispersal(swarm.get(i), task, i);
