...
public class PSOIS extends Algorithm {
	...
	int populationSize;
	ArrayList<PSOISSolution> population;
	PSOISSolution PgBest;
	double c1, c2, w;
	boolean improved;
	Task task;
		...
		while (!task.isStopCriteria()) {
			for (int i = 0; i < populationSize; i++) {
				...
				if (task.isStopCriteria())
					break;
				population.set(i, population.get(i).updateP(task, v, population.get(i)));
	...
	private void initPopulation() throws StopCriteriaException {
		...
		for (int i = 0; i < populationSize; i++) {
			population.add(new PSOISSolution(task));
			if (i == 0)
