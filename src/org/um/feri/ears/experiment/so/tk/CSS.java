...
//Opis algoritma se za�ne v poglavju 2.2 Presentation of charged search system
public class CSS extends Algorithm {
	...
	// Najboj�a re�itev in najslab�a re�itev v celotnem zagonu
	...
	public CSS(int pop, double par, double cmcr) {	
		...
		au = new Author("Tadej Klako�er", "tadej.klakocer@student.um.si");			
	...
	public DoubleSolution execute(Task taskProblem) throws StopCriteriaException {
		...
		while (!taskProblem.isStopCriteria()) {
			...
			// po vseh dimenzijah ista dol�ina intervala
			...
			// Premakne delce, �e so izven mej jih popravi z HS
			// Rule 5, 6 , 7 
			...
			// Update CM (�e bolj�i v CPs kot v CM, zamenjaj delce v CM)
	...
	// Izra�una magnitude of charge
	private void calcMagnitude() {
		// magnitude of charge
		for (int i = 0; i < pop_size; i++) {
			// Pride do deljenje z ni�, ker sta worst in best enaka, zato + eps
	...
	// Izra�una separation distance med delci
	...
	// Izra�una verjetnosti da se privla�ita
	private double[][] calcProb(Task taskProblem) {
		...
		for (int i = 0; i < pop_size; i++)
			for (int j = 0; j < pop_size; j++) {
				...
				// Ali ima j delec bolj�i fitnes od delca i ( f(j) > f(i))
	...
	// Izra�una total force, ki deluje na delec
	private void calcForce(Task taskProblem, double[][] Pij, double[][] rij) {

		for (int d = 0; d < taskProblem.getNumberOfDimensions(); d++) 
		{
			//Ena�ba 21, ampak deluje bolj�e �e je radij vsakega delca 1.0
			double a = 1.0;
			...
			for (int j = 0; j < pop_size; j++) {
				...
				for (int i = 0; i < pop_size; i++) {
					...
					// +eps, kr pride do deljenja z ni� in posledi�no potem NaN
					double part2 = (CPs.get(i).Q / (Math.pow(rij[j][i], 2) + eps)) * i2;

					suma_dim = suma_dim + ((part1 + part2) * desno);
				}

				// Kon�na vrednost za tega j-delca po d-dimenziji
				CPs.get(j).F[d] = suma_dim;//* CPs.get(j).Q;
			}
		}
	}

	// Premakne delce , novi velocity, HS based algorithm �e izven meja
	private void moveParticles(Task taskProblem, double kv, double ka) throws StopCriteriaException {
		...
		for (int j = 0; j < pop_size; j++) {
			...
			for (int d = 0; d < taskProblem.getNumberOfDimensions(); d++) {
				...
				// �e je dimenzija izven meja problema, Harmony search varjanta za novo pozicijo na tej dimenziji
				// RULE 7
			...
			if (harmonized[j]) 
			{
				...
				// Zamenjava najslab�ega v HM oz. CM.
				if (taskProblem.isFirstBetter(tmp2, CM.get(CM.size() - 1))) 
				{	
					// Najslab�ega zbri�e.
					CM.remove(CM.size() - 1);

					// Doda novega bolj�ega noter.
					CM.add(tmp2);
				}			
			}
	...
	// Poi��e novega najbolj�ega in najslab�ega.
	...
	private void refreshMemory(Task taskProblem) {
		for (int i = 0; i < CMS; i++) {
			...
			if (taskProblem.isFirstBetter(CPs.get(i), CM.get(CMS - 1))) {
				// po �lanku, �e sem dobro razumel
				CM.remove(CMS - 1); // slab�ega ven
				CM.add(CPs.get(i)); // bolj�ega noter
			}
		}
