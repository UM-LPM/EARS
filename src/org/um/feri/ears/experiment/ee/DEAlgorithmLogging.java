...
public class DEAlgorithmLogging extends Algorithm {
    ...
    public DEAlgorithmLogging(int strategy) {
        this(strategy, 30, Finit, CRinit);

    }

    public DEAlgorithmLogging(int strategy, int NP) {
        this(strategy, NP, Finit, CRinit);

    }

    public DEAlgorithmLogging(int strategy, int NP, double F, double CR) {
    ...
    public NumberSolution<Double> execute(Task taskProblem) throws StopCriterionException {
        ...
        while (!task.isStopCriterion()) {
            ...
            for (i = 0; i < NP; i++) /* Start of loop through ensemble */
            {
                ...
                NumberSolution<Double> br = task.eval(tmp, parents);
        ...
        if (maxCombinations == 1) {
            ...
            if (strategy == JDE_rand_1_bin) { //self adaptive no need for CR and F parameter
                ...
                for (int i = 0; (i < paramCombinations.length) && (counter < maxCombinations); i++) {
                    alternative.add(new DEAlgorithmLogging(strategy, paramCombinations[i]));
                    counter++;
            ...
            } else
            {
                ...
                for (int i = 0; (i < paramCombinations.length) && (counter < maxCombinations); i++) {
                    alternative.add(new DEAlgorithmLogging(strategy,(int) paramCombinations[i][0], paramCombinations[i][1], paramCombinations[i][2]));
                    counter++;
