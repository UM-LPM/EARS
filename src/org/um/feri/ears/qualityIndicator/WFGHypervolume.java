...
public class WFGHypervolume<T extends Number> extends QualityIndicator<T>{
	...
	public double evaluate(ParetoSolution<T> population) {
		...
		ParetoSolution<T> copy = new ParetoSolution<T>(population.getCapacity());
		
		for (MOSolutionBase<T> solution : population) {
		...
		this.maxNumberOfPoints = population.size();
