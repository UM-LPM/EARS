...
public class RealWorldCompressionSpringDesign extends Problem {
	...
	public double[] calc_constraints(double x[]) {
		...
		g[0] = 1 - ((Math.pow(x[1], 3) * x[2]) / (71785 * Math.pow(x[0], 4)));
		// TODO: Look into it. Velika razhajanja iz constrainta v članku in definiciji constrainta v http://www-optima.amp.i.kyoto-u.ac.jp/member/student/hedar/Hedar_files/TestGO_files/Page5161.htm
		g[1] = (4 * Math.pow(x[1], 2) - x[0] * x[1]) / (12566 * (x[1] * Math.pow(x[0], 3) - Math.pow(x[0], 4))) + (1 / (5108 * Math.pow(x[0], 2)));
