...
public class MeanStDev {
    ...
    public String toString2Lines(int devide) {
        StringBuffer sb = new StringBuffer();
        sb.append("\\begin{tabular}{@{}r@{}} ");
        if (stdev<0.000000000001) sb.append("$").append(meanFormat.format(mean/devide)+"$ \\\\ $\\pm 0$");
        else
            sb.append("$").append(meanFormat.format(mean/devide)+"$ \\\\ $\\pm"+stdFormat.format(stdev/devide)+"$");
        sb.append("\\end{tabular}");
        return sb.toString();
    }
